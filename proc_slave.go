...
var (
	...
	//simpleupdater is disabled and the program function
	//is run manually.
	DisabledState = State{Enabled: false}
)

// State contains the current run-time state of simpleupdater
type State struct {
	//whether simpleupdater is running enabled. When enabled,
	//this program will be running in a child process and
	//simpleupdater will perform rolling upgrades.
	Enabled bool
...
func (sp *slave) initFileDescriptors() error {
	...
	sp.listeners = make([]*simpleupdaterListener, numFDs)
...
func (sp *slave) debugf(f string, args ...interface{}) {
	if sp.Config.Debug {
		log.Printf("[simpleupdater slave#"+sp.id+"] "+f, args...)
	}
}
